...
- Scope: all three model runs (`claude_opus`, `codex-sol`, `cursor-grok`) plus the shared root files. Sections: Act 0-7, the arc, a standing direction, and load-bearing caveats.
...
- **Revision, same day, at user request.** The correction apparatus was removed from `war-story.md`. The 21 star markers, the "run X recorded Y, the card says Z" prose, and the Epilogue cross-run audit are gone. Every corrected fact stays, stated plainly on its own authority. This log entry is now the only record of what changed and why. Surviving meta-content is limited to three things: the sourcing note, Act 7 on the three tool failures, and caveats that name an open citation. Byte count moved from 33362 to 26286. A backup of the pre-revision file sits at `war-story.md.bak`.
